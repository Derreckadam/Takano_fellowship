...
ggplot(data=VetOver, aes(x=VetOver$Year, y=VetOver$Population, fill=Gender)) + theme(axis.text.x = element_text(angle = 90)) + ylab("Number of Veterans") + xlab("") + geom_bar(stat="identity") + labs(title="Riverside County Veteran Population") + geom_errorbar(aes(ymin=Population-SE, ymax=Population+SE), width=.1)
