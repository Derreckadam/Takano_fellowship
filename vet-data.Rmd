...
# History - 
In the 1970's the current VA's were established around Riverside. Jerry Lewis, Rep for Redlands, was on the appropriations committee. Which is probably how 5 locations were awarded to Loma Linda to oversee. 

# Veteran Population and Needs in Riverside County

This data is the US census data from 2015 to 2017 showing current breakdown of Veteran Population in Riverside County. We have a total of 140,000 veterans making up about 8% of the population. 
...
VetPop.plot <- ggplot(data=VetOver, aes(x=VetOver$Year, y=VetOver$Population, fill=Gender)) + theme(axis.text.x = element_text(angle = 90)) + ylab("Number of Veterans") + xlab("") + geom_bar(stat="identity") + labs(title="Riverside County Veteran Population") + geom_errorbar(aes(ymin=Population-SE, ymax=Population+SE), width=.1) +ylim(0, 150000)
...
p4 <- ggplot(income.here, aes(x=Age, y=income.here$`Population`, fill=Disability)) +
  geom_bar(stat="identity") + ylab("Individuals")  + ggtitle("District 41") 
...
```
```{r figs, echo=FALSE, fig.width=4,fig.height=4,fig.cap="This figure shows senior veterans (65+ years old) living below the poverty line in California Congressional Districts"}
p3
```
```{r figs, echo=FALSE, fig.width=4,fig.height=4,fig.cap="This figure shows veterans with and without disablilities living below the poverty line in California Congressional District 41"}
p4
```


```{r pressure, echo=FALSE, fig.cap="Veterans in Riverside County", out.width = '100%', fig.cap="A.) This figure shows veterans populations in each state"}
knitr::include_graphics("Veteran Population.png")

#change district border and Clinic Color
```
```{r pressure, echo=FALSE, out.width = '100%', fig.cap="B.) Blobs dot denoting locations within reach of VA Clinics"}
...
data.long
plot <- ggplot(data.long, aes(Location, Patients, fill=Space)) + geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = 90))
