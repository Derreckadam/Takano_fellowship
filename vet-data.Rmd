...
abstract: |
  This report briefly describes Veterans and thier needs in Southern Californai and how they differe from the greater united states. To analyze these questions the report utilizes many sources of data from the US Census Buerea, Veteran Affairs data, map data, and transit data. 

  In general this report finds that despite having the largest Veteran Population of any state it does not have A.) High Quality VA Clinics and Medical Centers and B.) Clinics that are accessilbe by much of the population. 
...
demo.plot <- ggplot(demo.m, aes(x=District, y=Race, size="Veteran Total", fill="Race")) + geom_jitter(aes(size = demo.m$`Veteran Total`, colour = demo.m$Race, alpha=.02)) + scale_size(range = c(0,5)) +
    theme_bw()

demo.plot

pdf("Demo.pdf", width = 10, height = 4)
  demo.plot
dev.off
...
```{r echo=FALSE, fig.cap="Veterans in Riverside County", out.width = '50%', fig.cap="A.) This figure shows veterans populations in california are the highest"}
...
dis <- read_excel("Data.xlsx", sheet = "B21100", range = "A1:E7")
dis.df <- as.data.frame(dis)

dis.m <- melt(dis.df, id = c("Service-connected Disability Rating", 'Riverside County',	'Riverside SE',	'District 41',	'District 41 SE'))

p4a <- ggplot(dis.m, aes(x=dis.m$`Service-connected Disability Rating`, y= dis.m$`Riverside County`, fill=dis.m$`Service-connected Disability Rating`)) + geom_bar(stat="identity", position = position_dodge()) + ylab("Individuals")  + ggtitle("Disablable Veteran Population in Riverside County") + theme(axis.text.x = element_text(angle = 90, vjust=.2)) + geom_errorbar(aes(ymin=dis.m$`Riverside County`-dis.m$`Riverside SE`, ymax=dis.m$`Riverside County`+dis.m$`Riverside SE`), width=.2, position=position_dodge(.9))

p4b <- ggplot(dis.m, aes(x=dis.m$`Service-connected Disability Rating`, y= dis.m$`District 41`, fill=dis.m$`Service-connected Disability Rating`)) + geom_bar(stat="identity", position = position_dodge()) + ylab("Individuals")  + ggtitle("Disablable Veteran Population in District 41") + theme(axis.text.x = element_text(angle = 90, vjust=.2)) + geom_errorbar(aes(ymin=dis.m$`District 41`-dis.m$`District 41 SE`, ymax=dis.m$`District 41`+dis.m$`District 41 SE`), width=.2, position=position_dodge(.9))
